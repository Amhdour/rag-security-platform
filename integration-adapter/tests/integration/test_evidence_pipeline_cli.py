...
def test_evidence_pipeline_cli_smoke(tmp_path) -> None:
    ...
    assert (root / "connectors.inventory.json").is_file()
    assert (root / "tools.inventory.json").is_file()
    assert (root / "mcp_servers.inventory.json").is_file()
